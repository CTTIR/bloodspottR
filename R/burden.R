check_nonnegative <- function(x, name, integer = FALSE) {
  if (!is.numeric(x) || anyNA(x) || any(!is.finite(x)) || any(x < 0))
    .bs_abort(name, " must be finite nonnegative numbers", call. = FALSE)
  if (integer && any(x != floor(x) | x > 2^53))
    .bs_abort(name, " must contain exact whole counts no larger than 2^53", call. = FALSE)
}
...
measure_burden <- function(n_spots, red_pixels, tissue_pixels,
  ...
  if (!n || length(n_spots) != n || length(red_pixels) != n)
    .bs_abort("Counts must have equal, nonzero lengths", call. = FALSE)
  ...
  expand_scale <- function(value, name) {
    ...
    if (!length(value) %in% c(1L, n) || any(value == 0))
      .bs_abort(name, " must be positive with length one or the measurement count", call. = FALSE)
  ...
  if (any(red_pixels > tissue_pixels))
    .bs_abort("Positive pixels exceed evaluable tissue", call. = FALSE)
  if (is.null(n_profiles)) {
    n_profiles <- rep(NA_real_, n)
  } else {
    if (length(n_profiles) != n) .bs_abort("n_profiles has incompatible length", call. = FALSE)
    check_nonnegative(n_profiles, "n_profiles", TRUE)
  }
  if (any(tissue_pixels == 0 & (n_spots > 0 | (!is.na(n_profiles) & n_profiles > 0))))
    .bs_abort("Object counts cannot be positive without evaluable tissue", call. = FALSE)
  if (any(n_spots > red_pixels) || any(red_pixels == 0 & !is.na(n_profiles) & n_profiles > 0))
    .bs_abort("Positive objects require supporting positive pixels", call. = FALSE)
  ...
  if (any(!is.finite(tissue_area_mm2) | !is.finite(red_area_mm2)) ||
    ...
    .bs_abort("Calibrated areas exceed finite positive numeric range", call. = FALSE)
  ...
  if (any(is.infinite(n_spots / denominator) | is.infinite(n_profiles / denominator)))
    .bs_abort("Calibrated densities exceed finite numeric range", call. = FALSE)
...
aggregate_burden <- function(x, by = c("physical_slide_id", "organ")) {
  ...
  if (any(by %in% reserved)) .bs_abort("Grouping columns conflict with generated metrics", call. = FALSE)
  ...
  if (!is.data.frame(x) || anyDuplicated(names(x)) || !nrow(x) || !is.character(by) || !length(by) ||
    ...
    .bs_abort("A nonempty measurement table with all required columns is needed", call. = FALSE)
  if (anyNA(x[c(by, "physical_area_id")]) || any(!nzchar(as.character(x$physical_area_id))) ||
    ...
    .bs_abort("Grouping and physical-area identifiers must be known", call. = FALSE)
  if (anyDuplicated(x$physical_area_id))
    .bs_abort("Duplicate physical_area_id; resolve repeated or overlapping measurements", call. = FALSE)
  ...
  if ("qc_flags" %in% names(x) && !is.character(x$qc_flags))
    .bs_abort("qc_flags must be a character column", call. = FALSE)
  if (any(x$red_area_mm2 > x$tissue_area_mm2) ||
      any(x$red_area_mm2 == 0 & (x$n_spots > 0 | (!is.na(x$n_profiles) & x$n_profiles > 0))))
    .bs_abort("Inconsistent tissue area or object counts", call. = FALSE)
  keys <- unique(x[by])
  rows <- lapply(seq_len(nrow(keys)), function(i) {
    ...
    if (!is.finite(area) || !is.finite(red) ||
      ...
      .bs_abort("Pooled measurements exceed finite numeric range", call. = FALSE)
