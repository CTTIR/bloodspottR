...
bs_backend <- function(command, args = character(), version,
  ...
  if (!is.character(command) || length(command) != 1L || is.na(command) || !nzchar(command))
    .bs_abort("command must identify one executable", call. = FALSE)
  executable <- if (file.exists(command)) normalizePath(command, winslash = "/") else Sys.which(command)
  if (!nzchar(executable) || dir.exists(executable) || file.access(executable, 1L) != 0L) .bs_abort("Executable not found", call. = FALSE)
  if (!is.character(args) || anyNA(args)) .bs_abort("args must be character", call. = FALSE)
  if (!is.character(version) || length(version) != 1L || is.na(version) || !nzchar(version))
    .bs_abort("An explicit backend version is required", call. = FALSE)
  if (!is.character(operations) || !length(operations) || anyNA(operations) ||
      any(!grepl("^[a-z][a-z0-9_]*$", operations)) || anyDuplicated(operations))
    .bs_abort("operations must be unique operation names", call. = FALSE)
...
bs_run_backend <- function(backend, operation, inputs, parameters = list(), out,
                           timeout = 3600) {
  if (!inherits(backend, "bs_backend") || !is.list(backend)) .bs_abort("Expected a bs_backend", call. = FALSE)
  ...
  if (!is.character(operation) || length(operation) != 1L || is.na(operation) ||
      !operation %in% backend$operations) .bs_abort("Unsupported operation", call. = FALSE)
  if (!is.character(inputs) || !length(inputs) || anyNA(inputs) ||
    ...
    .bs_abort("inputs must be named existing files", call. = FALSE)
  if (!is.list(parameters) || (length(parameters) &&
    ...
    .bs_abort("parameters must be a named list", call. = FALSE)
  ...
  if (!valid_parameter(parameters))
    .bs_abort("parameters must contain finite JSON-compatible values; use NULL for explicit null", call. = FALSE)
  if (!is.numeric(timeout) || length(timeout) != 1L || !is.finite(timeout) || timeout <= 0)
    .bs_abort("timeout must be finite and positive", call. = FALSE)
  if (!is.character(out) || length(out) != 1L || is.na(out) || !nzchar(out) ||
      file.exists(out) || !dir.exists(dirname(out))) .bs_abort("out must be a new path in an existing directory", call. = FALSE)
  if (!requireNamespace("processx", quietly = TRUE)) .bs_abort("Worker execution requires processx", call. = FALSE)
  ...
  process <- processx::run(backend$command, c(backend$args, request_file, normalizePath(stage)),
                           ...
                           stdout = "|", stderr = "|")
  if (isTRUE(process$timeout)) .bs_abort("Worker timed out; owned process tree cleaned up", call. = FALSE)
  if (process$status != 0L) .bs_abort(paste("Worker failed:", process$stderr), call. = FALSE)
  manifest <- file.path(stage, "result.json")
  if (!file.exists(manifest) || dir.exists(manifest)) .bs_abort("Worker did not produce result.json", call. = FALSE)
  ...
  if (any(!startsWith(normalizePath(staged_files, winslash = "/", mustWork = TRUE), prefix)) ||
    ...
    .bs_abort("Worker output contains links or paths escaping output directory", call. = FALSE)
  result <- jsonlite::read_json(manifest, simplifyVector = FALSE)
  if (!is.list(result) || !identical(result$status, "complete") ||
      !isTRUE(result$schema_version == 1L)) .bs_abort("Worker completion contract failed", call. = FALSE)
  ...
  if (!is.list(raw_files) || !is.null(names(raw_files)) || !length(raw_files) ||
    ...
    .bs_abort("Worker artifacts must be a nonempty array of paths", call. = FALSE)
  ...
  if (!is.character(files) || !length(files) || anyNA(files) || any(!nzchar(files)) || anyDuplicated(files) ||
    ...
    .bs_abort("Worker artifact paths are invalid", call. = FALSE)
  full <- file.path(stage, files)
  if (any(!file.exists(full)) || any(dir.exists(full))) .bs_abort("Worker artifacts are missing", call. = FALSE)
  resolved <- normalizePath(full, winslash = "/", mustWork = TRUE)
  prefix <- paste0(normalizePath(stage, winslash = "/"), "/")
  if (anyDuplicated(resolved)) .bs_abort("Worker artifact paths alias the same file", call. = FALSE)
  if (any(basename(resolved) == "job-receipt.json")) .bs_abort("Worker artifact uses reserved receipt name", call. = FALSE)
  if (any(!startsWith(resolved, prefix))) .bs_abort("Worker artifacts escape output directory", call. = FALSE)
  if (!file.exists(request_file) || !identical(request_hash, digest::digest(file = request_file, algo = "sha256")))
    .bs_abort("Worker changed the request manifest", call. = FALSE)
  after_hash <- vapply(inputs, digest::digest, "", file = TRUE, algo = "sha256")
  if (!identical(hashes, after_hash)) .bs_abort("Worker changed an input file", call. = FALSE)
  ...
  if (file.exists(out) || !file.rename(stage, out)) .bs_abort("Could not commit worker output", call. = FALSE)
