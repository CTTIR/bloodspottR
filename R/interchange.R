...
bs_as_cellspec <- function(events, images, provenance = list()) {
  ...
  for (nm in names(components)) {
    ...
    if (!is.data.frame(d) || anyDuplicated(names(d)) || !all(required[[nm]] %in% names(d)))
      .bs_abort(nm, " must contain unique named required columns: ", paste(required[[nm]], collapse = ", "), call. = FALSE)
    ...
    for (id in ids) if (!is.character(d[[id]]) || anyNA(d[[id]]) || any(!nzchar(trimws(d[[id]]))))
      .bs_abort(nm, "$", id, " must contain nonempty character identifiers", call. = FALSE)
  }
  if (anyDuplicated(images$image_id)) .bs_abort("image_id must be unique in images", call. = FALSE)
  if (anyDuplicated(events[c("image_id", "event_id")])) .bs_abort("event_id must be unique within image", call. = FALSE)
  for (nm in c("pixel_width_um", "pixel_height_um", "width_px", "height_px")) {
    v <- images[[nm]]
    if (!is.numeric(v) || any(!is.finite(v) | v <= 0)) .bs_abort(nm, " must be finite and positive", call. = FALSE)
  }
  for (nm in c("width_px", "height_px")) if (any(images[[nm]] != floor(images[[nm]]) | images[[nm]] > .Machine$integer.max))
    .bs_abort(nm, " must be positive integer dimensions", call. = FALSE)
  idx <- match(events$image_id, images$image_id)
  if (anyNA(idx)) .bs_abort("Every event image_id must occur in images", call. = FALSE)
  if (any(events$physical_slide_id != images$physical_slide_id[idx])) .bs_abort("physical_slide_id disagrees between events and images", call. = FALSE)
  for (nm in c("x_um", "y_um")) if (!is.numeric(events[[nm]]) || any(!is.finite(events[[nm]]) | events[[nm]] < 0))
    .bs_abort(nm, " must contain finite nonnegative coordinates", call. = FALSE)
  if (any(events$x_um >= images$width_px[idx] * images$pixel_width_um[idx] |
    ...
    .bs_abort("Event coordinates exceed the calibrated image extent", call. = FALSE)
  if (!is.list(provenance) || (length(provenance) && (is.null(names(provenance)) || anyNA(names(provenance)) || any(!nzchar(names(provenance))) || anyDuplicated(names(provenance)))) || "bloodspottR" %in% names(provenance))
    .bs_abort("provenance must be a uniquely named list without reserved bloodspottR entry", call. = FALSE)
  if (!requireNamespace("cellspecR", quietly = TRUE)) .bs_abort("Install optional package cellspecR to exchange event candidates", call. = FALSE)
